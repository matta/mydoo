...
///
/// Initializes a new [`PersistedTask`] using the provided unique `id` and `title`,
/// with default values (e.g., `Pending` status, default importance). If an optional `parent`
/// reference is provided, the new task inherits the parent's `place_id` and `credit_increment`.
///
/// # Examples
///
/// ```
