...
///
/// Initializes a [`PersistedTask`] using the provided `id` and `title`, applying default values (e.g., `Pending` status, default importance).
/// If an optional `parent` reference is provided, the new task inherits the parent's `place_id` and `credit_increment`.
/// Returns the new initialized [`PersistedTask`].
///
/// # Examples
///
/// ```
