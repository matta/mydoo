...
///
/// Initializes and returns a new [`PersistedTask`] with the given `id` and `title`.
/// The task starts with default values (e.g., `Pending` status, default importance).
/// If an optional `parent` reference is provided, the new task inherits the
/// parent's `place_id` and `credit_increment`.
///
/// # Examples
///
/// ```
