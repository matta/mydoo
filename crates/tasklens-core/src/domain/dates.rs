...
///
/// Returns the computed [`UrgencyStatus`] for the task. The urgency status is calculated relative to `current_time` (the current timestamp in ms) as follows:
/// - [`UrgencyStatus::Overdue`]: The `current_time` is strictly after `effective_due_date` (the timestamp when the task is due in ms), unless it's the same UTC day.
/// - [`UrgencyStatus::Urgent`]: The task's `effective_due_date` is today (UTC), or `time_remaining <= 0.25 * effective_lead_time`.
/// - [`UrgencyStatus::Active`]: The task is within its `effective_lead_time` window (the duration in ms before the due date when the task becomes active) (`time_remaining <= effective_lead_time`), but not yet urgent.
