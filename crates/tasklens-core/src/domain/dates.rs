...
/// - [`UrgencyStatus::Overdue`]: The `current_time` is strictly after the `effective_due_date` (the timestamp when the task is due in ms), unless it's the same UTC day.
...
///
/// The `effective_lead_time` parameter is the duration (in ms) before the due date when the task becomes active. The `current_time` parameter is the current timestamp (in ms).
