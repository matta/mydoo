...
///
/// The urgency status is computed based on the given `effective_due_date` timestamp (in milliseconds),
/// `effective_lead_time` duration (in milliseconds), and the `current_time` timestamp (in milliseconds).
///
/// # Examples
///
/// ```
