...
fn check_invariants(doc: &Automerge) -> Result<(), String> {
    ...
    // 2. Check Logical Invariants (Parent/Child consistency)
...
fn any_place_id() -> impl Strategy<Value = PlaceID> {
    prop_oneof![
...
fn any_task_updates() -> impl Strategy<Value = TaskUpdates> {
    (
        any::<Option<String>>(),
...
fn any_action() -> impl Strategy<Value = Action> {
    prop_oneof![
        ...
        (any_place_id(), any::<String>()).prop_map(|(_id, _name)| {
            // Places aren't fully handled by actions yet, but we use the ID strategy here to avoid warnings
            Action::RefreshLifecycle { current_time: 0 }
        }),
