<script lang="ts">
	...
	function canMarkDone(item: TodoItem): boolean {
		if (!item.children || !item.children.todoOrder || item.children.todoOrder.length === 0)
			return true;
		...
		{#if list && list.todoOrder}
			{#each list.todoOrder as id (id)}
				...
				{#if item}
					...
					{@const hasChildren =
						item.children && item.children.todoOrder && item.children.todoOrder.length > 0}
	...
	{:else if viewRootList}
		...
		<div class="breadcrumbs">
			{#each breadcrumbs as crumb, i}
				...
				{/if}
				<button class="breadcrumb" onclick={() => navigate(crumb.path)}>
					{crumb.title}
				</button>
			{/each}
		</div>
		...
		<div style="margin-top: 2rem; border-top: 1px solid #eee; padding-top: 1rem;">
			<button class="cleanup" onclick={cleanup}> Cleanup Completed Tasks </button>
		</div>
	{:else}
		<p>Initializing...</p>
	{/if}
