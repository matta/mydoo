...
/// Adds or updates the `mod.rs` with sorted `pub mod` lines and necessary pragmas.
fn add_component_module_line(components_root: &Path, component_name: &str) -> Result<()> {
    ...
    // Ensure standard pragmas are present
    let pragmas = [
    ...
    for pragma in pragmas.iter().rev() {
        if !lines.iter().any(|l| l.trim() == *pragma) {
            lines.insert(0, pragma.to_string());
        }
    }

    // Add the new component if missing
    let mod_line = format!("pub mod {};", component_name);
    if !lines.iter().any(|l| l.trim() == mod_line) {
        lines.push(mod_line);
    }

    // Reconstruct the file with sorted modules
    ...
    final_content.push('\n');
    final_content.push_str(&mod_lines.join("\n"));
    final_content.push('\n');

    fs::write(&mod_rs_path, final_content)
...
fn copy_component_files(
    ...
    while let Some(current_dir) = pending.pop() {
            ...
            .with_context(|| format!("failed to read {}", current_dir.display()))?
        {
            ...
            // Ensure trailing newline for text files to avoid "No newline at end of file" diffs.
            if let Some(ext) = destination_path.extension().and_then(|e| e.to_str()) {
                let text_extensions = ["rs", "css", "js", "json", "md", "toml", "html"];
                if text_extensions.contains(&ext) {
                    let content = fs::read_to_string(&destination_path).with_context(|| {
                        format!(
                            "failed to read {} for newline check",
                            destination_path.display()
                        )
                    })?;
                    if !content.is_empty() && !content.ends_with('\n') {
                        ...
                        fs::write(&destination_path, content).with_context(|| {
                            format!(
                                "failed to write {} after adding newline",
                                destination_path.display()
                            )
                        })?;
                    }
...
/// Ensures the component module directory and `mod.rs` file exist with standard header.
