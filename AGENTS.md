...
1.  **Generation Step:** BDD tests MUST be generated manually before execution. Tests are NOT automatically generated by `pnpm test`.
...
3.  **Stale Files:** If you delete or rename a `.feature` file, you MUST run `bddgen` to remove the corresponding stale `.spec.js` file. Stale files will cause `test-e2e` failures.
