...
- Adopted: 4 (Button, Navbar, Popover, Slider)
- Wrapper/Modified: 4 (Calendar, Collapsible, Dialog, Select)
- Diverged replacements: 3 (Checkbox, Date Picker, Input)
- Missing: 27
...
Deferred and tracked after Button component adoption:

- Raw RSX `button` elements still styled with DaisyUI `btn*` classes are present in:
...
- Additional DaisyUI `btn*` coupling remains in:
  - class strings passed to `Button { class: ... }` for size/shape tokens (for example `btn-sm`, `btn-circle`, `btn-xs`)
  - non-button list-option styling in `crates/tasklens-ui/src/app_components/move_picker.rs` and `crates/tasklens-ui/src/components/select/component.rs`
- These are intentionally deferred and are tracked in `dioxus-components-migration.todo.md` under `Deferred After Button Slice: DaisyUI Button Markup Cleanup`.
...
- Current next slice: Input (Button slice completed: vendored integration, legacy removal, and re-export update done).
...
- Re-vendor and replace diverged components first, one slice at a time: Input, then Checkbox, Date Picker.
