...
- [ ] Implement `close-settings` in `crates/tasklens-ui/src/views/settings_page.rs` to use `navigator().go_back()`; verify via `crates/tasklens-ui/tests/e2e/specs/settings-route-navigation.spec.ts`.
...
- [ ] Add a `Navigation Test` in `crates/tasklens-ui/tests/e2e/specs/settings-route-navigation.spec.ts` that verifies `close-settings` behavior.
