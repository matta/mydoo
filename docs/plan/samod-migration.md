...
- [x] **Phase 2: Store Logic & Compilation**
  ...
  - [x] Verify `tasklens-store` test coverage is _identical_ to what it was, by comparing the git diff and restoring missing tests.
...
- [ ] **Phase 4: E2E Testing & Verification**
  - [ ] Run `sync.spec.ts` (E2E tests).
    - _Note_: Sync server is now a Node.js script (`scripts/sync-server.js`) compatible with `automerge-repo`.
