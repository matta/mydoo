...
3.  **Privacy:** The server _only_ sees the `discovery_key` and opaque encrypted blobs. It cannot read the data or know the raw Document Secret.
...
- **Routing Logic:**
  ...
  - Route `SubmitChange` messages to all _other_ clients connected with the same `discovery_key`.
